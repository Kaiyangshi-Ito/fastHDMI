...
# os.chdir(sys.path[0]) # ensure working direcotry is set same as the filei-dimensional tensor;
    ...
    x = cp.abs(x)
    temp = cp.where(x<=lambda_, lambda_*x, cp.where(x<a*lambda_, (2*a*lambda_*x - x**2 - lambda_**2)/(2*(a - 1)), lambda_**2 * (a+1)/2))
...
def SCAD_grad(x, lambda_, a=3.7):
    ...
    sgn = cp.sign(x)
    x = cp.abs(x)
    temp = cp.where(x<=lambda_, lambda_*sgn, cp.where(x<a*lambda_, (a*lambda_*sgn-sgn*x)/(a-1), 0))
    temp[0] = 0. # this is to NOT penalize intercept beta later 
    return temp 
...
def MCP(x, lambda_, gamma):
    ...
    x = cp.abs(x)
    temp = cp.where(x<=gamma*lambda_, lambda_*x - x**2/(2*gamma), .5*gamma*lambda_**2)
    temp[0] = 0. # this is to NOT penalize intercept beta later 
    return temp 
...
def MCP_grad(x, lambda_, gamma):
    ...
    #x can be a multi-dimensional tensor. 
    '''
    temp = cp.where(cp.abs(x)<gamma*lambda_, lambda_*cp.sign(x)-x/gamma, cp.zeros_like(x))
    temp[0] = 0. # this is to NOT penalize intercept beta later 
    return temp 
...
def SCAD_concave(x, lambda_, a=3.7):
    ...
    '''
    x = cp.abs(x)
    temp = cp.where(x<=lambda_, 0., cp.where(x<a*lambda_, (lambda_*x - (x**2 + lambda_**2)/2)/(a-1), (a+1)/2*lambda_**2 - lambda_*x))
    temp[0] = 0. # this is to NOT penalize intercept beta later 
    return temp 
...
def SCAD_concave_grad(x, lambda_, a=3.7):
    ...
    sgn = cp.sign(x)
    x = cp.abs(x)
    temp = cp.where(x<=lambda_, 0., cp.where(x<a*lambda_, (lambda_*sgn-sgn*x)/(a-1), -lambda_*sgn))
    temp[0] = 0. # this is to NOT penalize intercept beta later 
    return temp 
...
def MCP_concave(x, lambda_, gamma):
    ...
    # similiar as in MCP
    x = cp.abs(x)
    temp = cp.where(x<=gamma*lambda_, -(x**2)/(2*gamma), (gamma*lambda_**2)/2 - lambda_*x)
    temp[0] = 0. # this is to NOT penalize intercept beta later 
    return temp 
...
#     y_temp /= cp.std(y)
    ...
    eliminated = cp.hstack((cp.array([False]), eliminated)) # because intercept coefficient is not penalized
    return eliminated
...
#     add design matrix column for the intercept, if it's not there already
    if add_intercept_column == True:
        if cp.any(X[:,0] != X[0,0]): # check if design matrix has included a column for intercept or not
...
#         L = cp.linalg.norm(cp.array([L_convex, 1./(a-1)]), ord=cp.infty)
        ...
        while ((not converged) or (k<3)) and k <= maxit:
            ...
            speed_norm = cp.linalg.norm(beta_md - beta_md_old, ord=2) # restarting
            converged = (cp.linalg.norm(beta_md - beta_md_old, ord=cp.infty) < tol)
...
#     add design matrix column for the intercept, if it's not there already
    ...
    if strongrule == True:
        ...
        elim = strong_rule_seq_LM(X, y, beta_old=cp.zeros(p_original), lambda_new=_lambda, lambda_old=_lambda_max)
...
def solution_path_LM_strongrule(design_matrix, outcome, lambda_, beta_0 = cp.ones(1), tol=1e-2, maxit=500, penalty="SCAD", a=3.7, gamma=2., add_intercept_column=True):
    ...
    _design_matrix = design_matrix.copy()
    if add_intercept_column == True:
        if cp.any(design_matrix[:,0] != design_matrix[0,0]): # check if design matrix has included a column for intercept or not
            intercept_design = cp.ones(N).reshape(-1, 1)
            _design_matrix = cp.hstack((intercept_design, _design_matrix))
    ...
    for j in range(len(lambda_)-1):
        ...
        _new_beta[cp.logical_not(elim)] = UAG_LM_SCAD_MCP(design_matrix=_design_matrix[:, cp.logical_not(elim)], outcome=outcome, beta_0 = _beta_0[cp.logical_not(elim)], tol=tol, maxit=maxit, _lambda=lambda_[j], penalty=penalty, a=a, gamma=gamma, add_intercept_column=False)[1]
...
#     add design matrix column for the intercept, if it's not there already
    ...
    if strongrule == True:
        ...
        p_original = X.shape[1]
        elim = strong_rule_seq_logistic(X, y, beta_old=cp.zeros(p_original), lambda_new=_lambda, lambda_old=_lambda_max)
        X = X[:, cp.logical_not(elim)]
        beta = beta[cp.logical_not(elim)]
...
def solution_path_logistic_strongrule(design_matrix, outcome, lambda_, beta_0 = cp.ones(1), tol=1e-2, maxit=500, penalty="SCAD", a=3.7, gamma=2., add_intercept_column=True):
    ...
    for j in range(len(lambda_)-1):
        ...
        _beta_0 = beta_mat[j,:]
        _new_beta = cp.zeros(_design_matrix.shape[1])
        _new_beta[cp.logical_not(elim)] = UAG_logistic_SCAD_MCP(design_matrix=_design_matrix[:, cp.logical_not(elim)], outcome=outcome, beta_0 = _beta_0[cp.logical_not(elim)], tol=tol, maxit=maxit, _lambda=lambda_[j], penalty=penalty, a=a, gamma=gamma, add_intercept_column=False)[1]
        beta_mat[j+1,:] = _new_beta
    return beta_mat[1:,:]
