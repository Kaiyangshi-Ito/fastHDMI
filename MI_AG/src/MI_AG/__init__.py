...
def _MI_continuous(a, b, a_min, a_max, N=500):
    ...
    _b0 = (b==0)
    if _np.sum(_b0)>2: # here proceed to kde only if there are more than 5 data points
...
def _MI_binary(a, b, a_min, a_max, N=500):
    ...
    _b0 = (b==0)
    if _np.sum(_b0)>2: # here proceed to kde only if there are more than 5 data points
        y_cond_p0 = lambda x: x*_np.sum(a[_b0]==1)/len(a[_b0]) + (1-x)*_np.sum(a[_b0]==0)/len(a[_b0])
#         y_cond_p0 = gaussian_kde(a[_b0])
    else:
        y_cond_p0 = _np.zeros_like
    _b1 = (b==1)
    if _np.sum(_b1)>2:
        y_cond_p1 = lambda x: x*_np.sum(a[_b1]==1)/len(a[_b1]) + (1-x)*_np.sum(a[_b1]==0)/len(a[_b1])
#         y_cond_p1 = gaussian_kde(a[_b1]) # this thing uses Scott's rule instead of Silverman defaulted by FFTKDE and R density
    else:
        y_cond_p1 = _np.zeros_like
    _b2 = (b==2)
    if _np.sum(_b2)>2:
        y_cond_p2 = lambda x: x*_np.sum(a[_b2]==1)/len(a[_b2]) + (1-x)*_np.sum(a[_b2]==0)/len(a[_b2])
#         y_cond_p2 = gaussian_kde(a[_b2])
    else:
        y_cond_p2 = _np.zeros_like
    joint = _np.empty((N,3))
...
def continuous_filter(bed_file, bim_file, fam_file, outcome, outcome_iid, a_min=100., a_max=250., N=500):
    ...
    """
    bed1 = _open_bed(filepath=bed_file, fam_filepath=fam_file, bim_filepath=bim_file)
    gene_iid = _np.array(list(bed1.iid))
    bed1_sid = _np.array(list(bed1.sid))
    outcome = outcome[_np.intersect1d(outcome_iid, gene_iid, assume_unique=True, return_indices=True)[1]]
    # get genetic indices
    ...
    for j in range(len(MI_UKBB)):
        ...
        MI_UKBB[j] = _MI_continuous(a=_outcome, b=_SNP, a_min = a_min, a_max = a_max, N=N)
    return MI_UKBB
...
def binary_filter(bed_file, bim_file, fam_file, outcome, outcome_iid, a_min=100., a_max=250., N=500):
    ...
    for j in range(len(MI_UKBB)):
        ...
        _outcome = outcome[_SNP != -127] # remove missing SNP in outcome
        _SNP = _SNP[_SNP != -127] # remove missing SNP
        MI_UKBB[j] = _MI_binary(a=_outcome, b=_SNP, a_min = a_min, a_max = a_max, N=N)
    return MI_UKBB
...
def continuous_filter_parallel(bed_file, bim_file, fam_file, outcome, outcome_iid, a_min=100., a_max=250., N=500, chunck_size=60000):
    ...
    def _continuous_filter_slice(_slice):
        ...
        for j in _slice:
            ...
            _outcome = outcome[_SNP != -127] # remove missing SNP in outcome
            _SNP = _SNP[_SNP != -127] # remove missing SNP
            _MI_slice[k] = _MI_continuous(a=_outcome, b=_SNP, a_min = a_min, a_max = a_max, N=N)
            k += 1
        return _MI_slice
    # multiprocessing starts here
    ind = _np.arange(len(bed1_sid))
    n_slices = _np.ceil(len(ind)/chunck_size)
    with _mp.Pool(_mp.cpu_count()) as p:
        MI_UKBB = p.map(_continuous_filter_slice, _np.array_split(ind, n_slices))
...
def binary_filter_parallel(bed_file, bim_file, fam_file, outcome, outcome_iid, a_min=100., a_max=250., N=500, chunck_size=60000):
    ...
    def _binary_filter_slice(_slice):
        ...
        for j in _slice:
            ...
            _outcome = outcome[_SNP != -127] # remove missing SNP in outcome
            _SNP = _SNP[_SNP != -127] # remove missing SNP
            _MI_slice[k] = _MI_binary(a=_outcome, b=_SNP, a_min = a_min, a_max = a_max, N=N)
            k += 1
        return _MI_slice
    # multiprocessing starts here
    ind = _np.arange(len(bed1_sid))
    n_slices = _np.ceil(len(ind)/chunck_size)
    with _mp.Pool(_mp.cpu_count()) as p:
        MI_UKBB = p.map(_binary_filter_slice, _np.array_split(ind, n_slices))
    MI_UKBB = _np.hstack(MI_UKBB)
    return MI_UKBB
...
#     y_temp /= _np.std(y)
    p=len(list(bed.sid))
    grad_at_0 = _SNP_update_smooth_grad_convex_LM(N=N, SNP_ind=SNP_ind, bed=bed, beta_md=_np.zeros(p), y=y, outcome_iid=outcome_iid)
    return lambda_max



# @_jit(nopython=True, cache=True, parallel=True, fastmath=True, nogil=True)
def SNP_UAG_LM_SCAD_MCP(bed_file, bim_file, fam_file, outcome, outcome_iid, SNP_ind, L_convex, beta_0 = _np.ones(1), tol=1e-2, maxit=500, _lambda=.5, penalty="SCAD", a=3.7, gamma=2.):
    '''
    Carry out the optimization for penalized LM for a fixed lambda.
    '''
...
# @_jit(nopython=True, cache=True, parallel=True, fastmath=True, nogil=True)
def SNP_solution_path_LM(bed_file, bim_file, fam_file, outcome, outcome_iid, lambda_, L_convex, SNP_ind, beta_0 = _np.ones(1), tol=1e-2, maxit=500, penalty="SCAD", a=3.7, gamma=2.):
    '''
    Carry out the optimization for the solution path without the strong rule.
    '''
    bed = _open_bed(filepath=bed_file, fam_filepath=fam_file, bim_filepath=bim_file)
...
# @_jit(nopython=True, cache=True, parallel=True, fastmath=True, nogil=True)
def SNP_UAG_LM_SCAD_MCP_parallel(bed_file, bim_file, fam_file, outcome, outcome_iid, SNP_ind, L_convex, beta_0 = _np.ones(1), tol=1e-2, maxit=500, _lambda=.5, penalty="SCAD", a=3.7, gamma=2., chunck_size=50000):
    '''
    Carry out the optimization for penalized LM for a fixed lambda.
    '''
    bed = _open_bed(filepath=bed_file, fam_filepath=fam_file, bim_filepath=bim_file)
...
# @_jit(nopython=True, cache=True, parallel=True, fastmath=True, nogil=True)
def SNP_solution_path_LM_parallel(bed_file, bim_file, fam_file, outcome, outcome_iid, lambda_, L_convex, SNP_ind, beta_0 = _np.ones(1), tol=1e-2, maxit=500, penalty="SCAD", a=3.7, gamma=2., chunck_size=50000):
    '''
    Carry out the optimization for the solution path without the strong rule.
    '''
    bed = _open_bed(filepath=bed_file, fam_filepath=fam_file, bim_filepath=bim_file)
...
#     y_temp /= _np.std(y)
    p=len(list(bed.sid))
    grad_at_0 = _SNP_update_smooth_grad_convex_logistic(N=N, SNP_ind=SNP_ind, bed=bed, beta_md=_np.zeros(p), y=y, outcome_iid=outcome_iid)
    return lambda_max



# @_jit(nopython=True, cache=True, parallel=True, fastmath=True, nogil=True)
def SNP_UAG_logistic_SCAD_MCP(bed_file, bim_file, fam_file, outcome, outcome_iid, SNP_ind, L_convex, beta_0 = _np.ones(1), tol=1e-2, maxit=500, _lambda=.5, penalty="SCAD", a=3.7, gamma=2.):
    '''
...
# @_jit(nopython=True, cache=True, parallel=True, fastmath=True, nogil=True)
def SNP_solution_path_logistic(bed_file, bim_file, fam_file, outcome, outcome_iid, lambda_, L_convex, SNP_ind, beta_0 = _np.ones(1), tol=1e-2, maxit=500, penalty="SCAD", a=3.7, gamma=2.):
    '''
    Carry out the optimization for the solution path without the strong rule.
    '''
    bed = _open_bed(filepath=bed_file, fam_filepath=fam_file, bim_filepath=bim_file)
...
#     y_temp -= _np.mean(y)
#     y_temp /= _np.std(y)
    grad_at_0 = _SNP_update_smooth_grad_convex_logistic_parallel(N=N, SNP_ind=SNP_ind, bed=bed, beta_md=_np.zeros(len(SNP_ind)), y=y, outcome_iid=outcome_iid, chunck_size=chunck_size)
    return lambda_max



# @_jit(nopython=True, cache=True, parallel=True, fastmath=True, nogil=True)
def SNP_UAG_logistic_SCAD_MCP_parallel(bed_file, bim_file, fam_file, outcome, outcome_iid, SNP_ind, L_convex, beta_0 = _np.ones(1), tol=1e-2, maxit=500, _lambda=.5, penalty="SCAD", a=3.7, gamma=2., chunck_size=50000):
    '''
    Carry out the optimization for penalized logistic for a fixed lambda.
    '''
    bed = _open_bed(filepath=bed_file, fam_filepath=fam_file, bim_filepath=bim_file)
...
# @_jit(nopython=True, cache=True, parallel=True, fastmath=True, nogil=True)
def SNP_solution_path_logistic_parallel(bed_file, bim_file, fam_file, outcome, outcome_iid, lambda_, L_convex, SNP_ind, beta_0 = _np.ones(1), tol=1e-2, maxit=500, penalty="SCAD", a=3.7, gamma=2., chunck_size=50000):
    '''
    Carry out the optimization for the solution path without the strong rule.
    '''
    bed = _open_bed(filepath=bed_file, fam_filepath=fam_file, bim_filepath=bim_file)
