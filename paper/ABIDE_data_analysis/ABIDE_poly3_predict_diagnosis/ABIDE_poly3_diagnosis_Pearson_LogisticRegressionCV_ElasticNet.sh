...
#SBATCH --job-name=poly3_diagnosis_Pearson_LogisticRegressionCV_ElasticNet
...
# ### built fastHDMI 1.18.15
# ${HOME}/wheels_builder/unmanylinuxize.sh --package fastHDMI --version 1.18.15 --python 3.8,3.9,3.10 --find_links https://files.pythonhosted.org/packages/
...
pip install --no-index /home/kyang/fastHDMI-1.18.15+computecanada-py3-none-any.whl

nvidia-smi
lscpu

echo "running ABIDE_poly3_diagnosis_Pearson_LogisticRegressionCV_ElasticNet.py"
...
python3 ABIDE_poly3_diagnosis_Pearson_LogisticRegressionCV_ElasticNet.py
