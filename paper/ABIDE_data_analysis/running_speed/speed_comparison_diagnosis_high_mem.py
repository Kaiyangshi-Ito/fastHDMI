...
def _get_computing_time(prop_input_vars):
    s = '''mi.binary_screening_csv_parallel(csv_file,_usecols=abide_name.copy()[0:int(len(abide_name)*prop_input_vars)],csv_engine="c",sample=1250000,multp=10,core_num=16,share_memory=False,kernel="epa",bw="ISJ",verbose=0)'''
...
print(
    "Running speed comparison when the outcome variable is diagnosis (binary) and the memory setting is high_mem... "
)
