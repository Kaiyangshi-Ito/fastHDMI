...
def _get_computing_time(prop_input_vars):
    s = '''mi.continuous_screening_csv_parallel(csv_file, _usecols=abide_name.copy()[0:int(len(abide_name)*prop_input_vars)], csv_engine="c", sample=1250000, multp=10, core_num=16, share_memory=True, kernel="epa", bw="ISJ", norm=2)'''
    ...
    s = '''mi.continuous_skMI_screening_csv_parallel(csv_file, _usecols=abide_name.copy()[0:int(len(abide_name)*prop_input_vars)], csv_engine="c", sample=1250000, multp=10, core_num=16, random_state=0, share_memory=True)'''
    imports_and_vars = globals()
    imports_and_vars.update(locals())
    num_loops = timeit.Timer(stmt=s, globals=imports_and_vars).autorange()[0]
